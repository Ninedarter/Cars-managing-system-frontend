...
export class ExpencesComponent {
  ...
  createAllChart(){
   const monthlyTotals = this.calculateMonthlyMaintenanceCosts(this.maintenances);
  ...
  public getMaintencesByCar(vinCode: string) {
    ...
    this.httpClient
      .get<Maintenance[]>('http://localhost:8082/maintenances/', { headers, params })
      .subscribe(
        (response: any) => {
  ...
  calculateMonthlyMaintenanceCosts(maintenances : Maintenance[]) : { [month: string]: number }  {
